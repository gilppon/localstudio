  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8000/api/generate/img2video', {
        ...
        body: JSON.stringify({ prompt, image_base64: image, motion_strength: motionStrength, model: "Wan I2V 14B" })
      });
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2">
            이미지-비디오 (I2V) 스튜디오 <span className="text-xs px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/30">Wan I2V</span>
          </h1>
